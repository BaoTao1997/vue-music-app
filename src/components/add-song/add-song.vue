    ...
    <div class="add-song" v-show="showFlag" @click.stop>
      ...
      <div class="search-result" v-show="query">
        <suggest :query="query" :showSinger="showSinger"
                 @listScroll="blurInput" @select="selectSuggest"></suggest>
      </div>
      <top-tip ref="topTip">
        <div class="tip-title">
          <i class="icon-ok"></i>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      show() {
        // 一开始add-song组件是display:none,所以scroll无法滚动,需要在add-song渲染时重新计算scroll的高度
        this.showFlag = true
        setTimeout(() => {
      ...
      selectSong(song, index) {
        if (index !== 0) {
          // 因为Song是从缓存取的不是Song对象,所以用new来创建Song对象
